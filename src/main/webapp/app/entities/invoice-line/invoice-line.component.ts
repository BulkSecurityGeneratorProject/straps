...
export class InvoiceLineComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.invoiceLineService
                ...
                .subscribe(
                    (res: HttpResponse<IInvoiceLine[]>) => this.paginateInvoiceLines(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/invoice-line'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IInvoiceLine) {
        return item.id;
    }
    ...
    protected paginateInvoiceLines(data: IInvoiceLine[], headers: HttpHeaders) {
