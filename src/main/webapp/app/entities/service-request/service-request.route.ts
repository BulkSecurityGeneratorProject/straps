    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<ServiceRequest> {
        ...
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<ServiceRequest>) => response.ok),
                map((serviceRequest: HttpResponse<ServiceRequest>) => serviceRequest.body)
            );
        }
...
export const serviceRequestRoute: Routes = [
    {
        ...
        data: {
            ...
            defaultSort: 'id,asc',
            pageTitle: 'promalyV5App.serviceRequest.home.title'
        },
